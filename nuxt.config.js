...
export default {
  ...
  feed: [
  {
    ...
    async create(feed) {
      ...
      feed.options = {
        title: 'Dusty Rose Piano',
        description: 'Piano Tuning Service in Utah',
        link: 'https://www.dustyrosepiano.com/rss.xml',
      };
      ...
      posts.forEach((post) => {
        ...
        const url = `https://dustyrosepiano.com/blog/${post.slug}`;
        const img = `https://dustyrosepiano.com/${post.img}`;
        ...
        if (typeof(post.canonical) == 'undefined') {
          canon = url
        } else {
          canon = post.canonical
          if (canon != url) indexit = false
        }
